...
class SidebarWidget(QWidget):
    ...
    def __init__(self) -> None:
        ...
        self.platform_island.content_layout.addWidget(self.combo_source, 0, Qt.AlignmentFlag.AlignHCenter)
    ...
    def _make_nav_item(self, page: dict[str, str]) -> NavItemWidget:
        ...
        self._page_titles[page["id"]] = page["title"]
        return item
    ...
    def _build_platform_combo(self) -> QComboBox:
        ...
        self._apply_platform_combo_style(combo)
        return combo

    def _apply_platform_combo_style(self, combo: QComboBox | None = None) -> None:
        target = combo or getattr(self, "combo_source", None)
        if target is None:
            return
        c = self._colors
        target.setStyleSheet(
            f"""
            QComboBox#PlatformSourceCombo {{
                min-height: 40px;
            ...
            QComboBox#PlatformSourceCombo:hover {{
                border-color: {c["accent"]};
                background: {c["input"]};
            }}
            QComboBox#PlatformSourceCombo:focus,
            QComboBox#PlatformSourceCombo:on {{
                border: 2px solid {c["accent"]};
                background: {c["input"]};
            }}
            QComboBox#PlatformSourceCombo::drop-down {{
                border: none;
                width: 0px;
            }}
            QComboBox#PlatformSourceCombo::down-arrow {{
                image: none;
                border: none;
                width: 0px;
                height: 0px;
            }}
            QComboBox#PlatformSourceCombo QAbstractItemView {{
                background: {c["panel"]};
                color: {c["text"]};
                border: 2px solid {c["accent"]};
                border-radius: 7px;
            ...
            QComboBox#PlatformSourceCombo QAbstractItemView::item {{
                border: none;
                min-height: 36px;
                padding: 4px 9px;
            }}
            QComboBox#PlatformSourceCombo QAbstractItemView::item:selected {{
                background: transparent;
                border: none;
                color: #ffffff;
            }}
            QComboBox#PlatformSourceCombo QAbstractItemView::item:selected:hover {{
                background: transparent;
                border: none;
                color: #ffffff;
            }}
            QComboBox#PlatformSourceCombo QAbstractItemView::item:hover {{
                background: transparent;
                border: none;
                color: {c["accent"]};
            }}
    ...
    def refresh_theme(self, is_dark: bool | None = None) -> None:
        ...
        self._apply_platform_combo_style()
