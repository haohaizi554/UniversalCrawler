...
```
GUI:  btn_dir.clicked
      → QFileDialog.getExistingDirectory(self, "选择保存目录", current_save_dir)
      → if selected_dir:
...
GUI:  btn_theme.clicked → toggle_theme()
      ...
      → cfg.set("common", "dark_theme", is_dark_theme)
      → cfg.set("common", "theme", "dark" if is_dark_theme else "light")
...
| 保存到配置 | `cfg.set()` | `sendWS('change_theme')` → 服务端 `cfg.set()` | ✅ |
...
GUI:
  btn_dir.clicked → on_dir_clicked()
      → selected_dir = QFileDialog.getExistingDirectory(self, "选择保存目录", current_save_dir)
      → if selected_dir:
...
| `change_theme` | 前端→服务端 | `{dark_theme}` | `cfg.set()` | ❌ 不需要 | ✅ |
...
**1. DownloadWorker.run() 调整顺序**（[download_manager.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/core/download_manager.py)）：
```python
def run(self):
    ...
    # 先把保存目录和目标文件名计算清楚
...
**2. WebController._on_task_started 携带 local_path**（[controller.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/web/controller.py)）：
```python
def _on_task_started(self, video_id: str):
...
**3. 前端 task_started 事件处理同步 local_path**（[index.html](file:///d:/desktop/project/UniversalCrawlerProplus/app/web/static/index.html)）：
```javascript
case 'task_started':
...
**修复**（[index.html](file:///d:/desktop/project/UniversalCrawlerProplus/app/web/static/index.html)）：
```css
...
**修复**（[controller.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/web/controller.py#L50-L88)）：
```python
...
并且 [server.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/web/server.py) WebSocket 连接时设置 loop：
```python
...
**Web 方案**（[server.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/web/server.py)）：
```python
...
**Web 前端调用**（[index.html](file:///d:/desktop/project/UniversalCrawlerProplus/app/web/static/index.html)）：
```javascript
...
**1. 禁用浏览器原生右键菜单**（[index.html](file:///d:/desktop/project/UniversalCrawlerProplus/app/web/static/index.html)）：
```javascript
...
**为什么这样修**：
- `spider.stop()` 内部已经发 "🛑 正在停止任务..."（在 [base.py:34](file:///d:/desktop/project/UniversalCrawlerProplus/app/spiders/base.py#L34)）
...
**修复**（[app/web/controller.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/web/controller.py)）：
```python
def stop_crawl(self):
...
# 3. 重新启动
cd d:\desktop\project\UniversalCrawlerProplus
python web_main.py
```
...
**修复点 1：BaseSpider 加 `interruptible_sleep()`**（[app/spiders/base.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/spiders/base.py)）：
...
**修复点 2：BaseSpider.stop() 强制关闭 Playwright**（[app/spiders/base.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/spiders/base.py)）：
...
**修复点 3：MissAVSpider 暴露 browser 引用**（[app/spiders/missav/spider.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/spiders/missav/spider.py)）：
...
**修复点 4：controller.stop_crawl() 主动 emit 日志**（[app/web/controller.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/web/controller.py)）：
...
GUI 端**没有"主图切换"按钮**——`ApplicationController.play_video()` 内部会按文件类型自动调用 `show_image()` 或 `play_video()`。Web 端的 [previewVideo()](file:///d:/desktop/project/UniversalCrawlerProplus/app/web/static/index.html#L1430) 已经在做图片/视频自动切换。

**我理解的"主图切换"= 增强功能**：在播放控制区加 `⏮` `⏭` 按钮，循环切换下载队列中的资源，**等效于用户在桌面 GUI 不断点击下载队列中的 ▶ 播放按钮**。

### 33.4 修复 v14：移除 🧪 测试弹窗按钮 + 加 ⏮/⏭ 主图切换

**修复点 1：移除调试按钮**（[app/web/static/index.html](file:///d:/desktop/project/UniversalCrawlerProplus/app/web/static/index.html)）：
...
| 切换目录 | `btn_dir.clicked` → `QFileDialog.getExistingDirectory` | `onChangeDirClicked()` → `pick-native` (PowerShell) → `showDirDialog()` (web 浏览器) | ✅ |
...
> 用户反馈原话："💥 爬虫错误: [Errno 13] Permission denied: 'C:\\Users\\MemoryLeak\\AppData\\Local\\UniversalCrawlerPro\\logs\\latest_debug.log'"
...
**问题**：
- 三个进程都 `import debug_logger` → 触发 [DebugLogger()](file:///d:/desktop/project/UniversalCrawlerProplus/app/debug_logger.py#L41) 单例
...
**修复点 1：进程判断 + session_file 命名带进程名**（[app/debug_logger.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/debug_logger.py#L41-L62)）：
...
**修复点 2：`_safe_write_text()` try/except + 重试**（[app/debug_logger.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/debug_logger.py#L64-L80)）：
...
**修复点 3：`_append_lines()` 子进程不写 latest_file**（[app/debug_logger.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/debug_logger.py#L141-L165)）：
...
**修复点 4：`_write_error_summary()` 也走 `_safe_write_text`**（[app/debug_logger.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/debug_logger.py#L290-L293)）：
...
| 6 | 更改目录 | `btn_dir.clicked` → `QFileDialog.getExistingDirectory` | `onChangeDirClicked()` → `showDirDialog()` (Web 目录浏览器) | ✅ v16修 |
...
1. **GUI 端流程**（[app/controllers/application_controller.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/controllers/application_controller.py)）：
   - `on_btn_dir_clicked()` → `QFileDialog.getExistingDirectory()` → `self.current_save_dir = dir` → `cfg.set(...)` → `sig_change_dir.emit()`
...
**新增 `async_scan_local_dir` 方法**（[app/web/controller.py](file:///d:/desktop/project/UniversalCrawlerProplus/app/web/controller.py)）：
...
>
> 用户第二次反馈原话："🔗 WebSocket 已连接 / 📂 正在扫描目录: D:\desktop\lly\vlog / ✅ 已加载 194 个本地文件 / 📂 正在切换目录: D:\desktop\lly\颠 / 看到了没，前端根本就不更新"
...
| 34 | 主题切换后写 cfg | `cfg.set("common", "dark_theme", ...); cfg.set("common", "theme", "dark"/"light")` | 后端 `change_theme` 消息处理同样双写 | ✅ | 完全一致 |
...
| 67 | 文件夹选择器 | QFileDialog.getExistingDirectory | 自实现 dirModal（驱动器 + 上一级 + 刷新 + 路径输入） | ✅ | Web 端无原生 QFileDialog，自实现对等体验 |
...
Start-Process -FilePath "python" -ArgumentList "web_main.py" -WorkingDirectory "d:\desktop\project\UniversalCrawlerProplus" -WindowStyle Hidden
...
| 扫描目录 | `python -m cli scan "D:\desktop" --limit 5 --pretty` | 成功返回 1 个文件 ✅ |
