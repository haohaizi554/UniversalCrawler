...
class ConnectionManager:
    ...
    async def _emit_to_connections(
        ...
        for conn in list(connections):
            await self._enqueue(conn, message)
    ...
    async def _enqueue(self, conn: WebSocketConnection, message: OutboundMessage) -> None:
        async with conn.queue_lock:
            ...
            if len(conn.outbound_queue) >= self._max_queue_size:
                if message.priority == FrontendEventPriority.NOISY:
                    ...
                    return
                if not self._drop_queued_noisy(conn):
    ...
    def _replace_coalesced(conn: WebSocketConnection, message: OutboundMessage) -> bool:
        if not message.coalesce_key[1] and message.event_type not in {"frontend_delta", "log"}:
            return False
        for index, queued in enumerate(conn.outbound_queue):
            if queued.coalesce_key == message.coalesce_key and queued.priority == FrontendEventPriority.NOISY:
                conn.outbound_queue[index] = message
                return True
        return False

    @staticmethod
    def _drop_queued_noisy(conn: WebSocketConnection) -> bool:
        for queued in list(conn.outbound_queue):
            if queued.priority == FrontendEventPriority.NOISY:
