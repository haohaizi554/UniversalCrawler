...
const FALLBACK_UI_TEXT = {
  "en-US": {
    ...
    "选择要安装的修订": "Select revision to install",
    "初始版": "Initial release",
    "修订": "Revision",
    "跳过此修订": "Skip this revision",
    "已跳过此修订": "This revision has been skipped",
    "跳过更新失败": "Failed to skip update",
    "所选修订不高于当前安装版本。继续将执行回退或重装，是否继续？": "The selected revision is not newer than the installed release. Continue with downgrade or reinstall?",
    "已取消回退或重装": "Downgrade or reinstall cancelled",
    ...
    "下载完成打开方式": "Completed-download open mode",
    "显示浏览器内核": "Show browser engine",
    "MissAV 有 5 秒盾，建议显式运行并手动过盾": "MissAV has a 5-second shield; run visibly and pass it manually",
    ...
    "自动打开，开启时使用": "Used when auto-open is enabled",
    "任务完成后自动打开": "Open automatically after completion",
    "非登录流程是否弹出浏览器窗口": "Whether non-login browser work opens a visible window",
    ...
    "手动播放方式": "Manual playback mode",
    "点击播放键时使用": "Used when clicking Play",
    ...
    "暂无参数": "No parameters",
    "暂无执行结果": "No result yet",
    "暂无使用历史": "No history",
    "暂无": "Not available",
    "请输入参数": "Enter a value",
    "必填项": "Required",
    "请输入有效数字": "Enter a valid number",
    "数值低于允许范围": "Value is below the allowed range",
    "数值超过允许范围": "Value exceeds the allowed range",
    "请选择有效选项": "Select a valid option",
    "输入格式不正确": "Invalid format",
    "请修正标记的参数": "Fix the highlighted parameters",
    "参数验证通过": "Parameters are valid",
    ...
    "已启用": "Enabled",
    "未启用": "Disabled",
    "操作提交失败": "Failed to submit action",
    "操作正在进行": "An action is in progress",
    "当前没有可取消的运行": "There is no active run to cancel",
    "工具运行": "Tool run",
    "今天": "Today",
  ...
  "zh-TW": {
    ...
    "已跳过此修订": "已跳過此修訂",
    "跳过更新失败": "跳過更新失敗",
    "所选修订不高于当前安装版本。继续将执行回退或重装，是否继续？": "所選修訂不高於目前安裝版本。繼續將執行回退或重裝，是否繼續？",
    "已取消回退或重装": "已取消回退或重裝",
    ...
    "下载完成打开方式": "下載完成開啟方式",
    "显示浏览器内核": "顯示瀏覽器核心",
    "MissAV 有 5 秒盾，建议显式运行并手动过盾": "MissAV 有 5 秒盾，建議顯式執行並手動通過",
    "绑定默认打开方式": "綁定預設開啟方式",
    "保存下载文件的位置": "保存下載檔案的位置",
    "从预设模板中选择": "從預設範本中選擇",
    "下载完成后的打开行为": "下載完成後的開啟行為",
    "自动打开，开启时使用": "開啟自動打開時使用",
    "任务完成后自动打开": "任務完成後自動開啟",
    "非登录流程是否弹出浏览器窗口": "非登入流程是否彈出瀏覽器視窗",
    "最大同时下载数": "最大同時下載數",
    "控制图片快车道": "控制圖片快車道",
    "网络请求等待时间": "網路請求等待時間",
    "失败后重试次数": "失敗後重試次數",
