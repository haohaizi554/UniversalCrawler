...
class DebugLogger:
    ...
    def __init__(self):
        ...
        self._min_level = self.LEVEL_ORDER["INFO"]
        ...
        *,
        ...
        if retention_days is not None:
            try:
                ...
                self._retention_days = days if days in _VALID_RETENTION_DAYS else _DEFAULT_RETENTION_DAYS
            except (TypeError, ValueError):
                self._retention_days = _DEFAULT_RETENTION_DAYS
        if cleanup_old_logs:
            policy = (self._retention_days, datetime.now().strftime("%Y-%m-%d"))
            if policy != self._last_cleanup_policy:
                self.cleanup_old_logs(self._retention_days)
    ...
    def cleanup_old_logs(self, retention_days: int | None = None) -> int:
        ...
        keep_paths = {self.session_file.resolve(), self.latest_file.resolve(), self.latest_error_summary_file.resolve()}
        removed = 0
        for path in self.logs_dir.glob("debug_*.log"):
            try:
                resolved = path.resolve()
                if resolved in keep_paths or path.stat().st_mtime >= cutoff:
                    continue
                path.unlink()
                removed += 1
            except OSError:
    ...
    def _should_write_level(self, level: str) -> bool:
