"""抖音底层能力模块，负责 `app/core/lib/douyin/tools/format.py` 对应的接口、加密、提取或工具逻辑。"""

# app/core/lib/douyin/tools/format.py
from http.cookiejar import CookieJar
from re import compile

def cookie_str_to_dict(cookie_str: str) -> dict:
    """执行 `cookie_str_to_dict` 对应的业务逻辑。"""
    if not cookie_str:
        return {}
    cookie = {}
...
def cookie_jar_to_dict(cookie_jar: CookieJar) -> dict:
    """执行 `cookie_jar_to_dict` 对应的业务逻辑。"""
    return {i.name: i.value for i in cookie_jar}

def format_size(size_in_bytes: int) -> str:
    """执行 `format_size` 对应的业务逻辑。"""
