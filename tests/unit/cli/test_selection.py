...
class RuleSelectionTests(unittest.TestCase):
    ...
    def test_select_with_invalid_token_is_rejected(self):
        """select 拼写错误必须明确失败，绝不能静默扩大为全选。"""
        from shared.selection_runtime import RuleSelection

        with self.assertRaisesRegex(ValueError, "abc"):
            RuleSelection(select="0,abc,2")

    def test_select_with_only_out_of_range_indices_is_rejected(self):
        """全部越界的显式选择不能退化为全选。"""
        from shared.selection_runtime import RuleSelection

        selection = RuleSelection(select="99")
        with self.assertRaisesRegex(ValueError, "有效范围"):
            selection.select([{"index": i} for i in range(3)])

    def test_exclude_with_only_out_of_range_indices_is_rejected(self):
        """完全无效的显式排除不能留下默认全选结果。"""
        from shared.selection_runtime import RuleSelection

        selection = RuleSelection(exclude="99")
        with self.assertRaisesRegex(ValueError, "有效范围"):
            selection.select([{"index": i} for i in range(3)])
...
class SelectionSummaryHelpersTests(unittest.TestCase):
    ...
    def test_format_selection_result_truncates_preview_when_needed(self):
        ...
        from shared.selection_runtime import format_selection_result
