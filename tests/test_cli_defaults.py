...
class PlatformDefaultsTests(unittest.TestCase):
    ...
    def test_xiaohongshu_download_defaults_include_headers(self):
        ...
        with patch("shared.runtime_options._try_load_cookie", return_value=None):
...
class ValidateConfigTypesTests(unittest.TestCase):
    ...
    def test_valid_max_items(self):
        """max_items=int 必须通过。"""
        from shared.runtime_options import validate_config_types
    ...
    def test_bool_is_rejected_for_numeric_config(self):
        """所有数值联合类型同样必须显式排除 bool。"""
        from shared.runtime_options import validate_config_types

        err = validate_config_types({"timeout": True})
        self.assertIsNotNone(err)
...
class MissAVProxyBuildTests(unittest.TestCase):
    ...
    def test_clash_alias(self):
        ...
        from shared.runtime_options import build_missav_proxy_url
        self.assertEqual(
...
class FallbackConfigTests(unittest.TestCase):
    ...
    def test_fallback_config_has_all_platforms(self):
        """兜底配置必须覆盖所有平台。"""
        from shared.runtime_options import _FALLBACK_CONFIG
    ...
    def test_default_config_alias(self):
        ...
        from shared.runtime_options import DEFAULT_CONFIG, _FALLBACK_CONFIG
