...
class MkvPlaybackRepairServiceTests(unittest.TestCase):
    ...
    def test_run_process_with_progress_kills_ffmpeg_after_timeout(self):
        process = Mock()
        process.poll.return_value = None
        process.communicate.return_value = ("", "")

        with patch("app.services.mkv_repair_service.subprocess.Popen", return_value=process), patch(
            "app.services.mkv_repair_service.time.monotonic",
            side_effect=[
                100.0,
                100.0 + MkvPlaybackRepairService.REPAIR_PROCESS_TIMEOUT_SECONDS,
            ],
        ):
            result = MkvPlaybackRepairService._run_process_with_progress(
                ["ffmpeg", "-i", "in.mkv", "out.mkv"],
                temp_path=Path("out.mkv"),
                source_size=1,
