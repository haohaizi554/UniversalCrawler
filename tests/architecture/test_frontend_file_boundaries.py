...
from tests.support.frontend_static_assets import stylesheet_names_from_index
...
BROWSER_CASE_DIR = TESTS_DIR / "support" / "browser_cases"
TEST_SUPPORT_MODULES = (
    TESTS_DIR / "support" / "browser_runtime.py",
    TESTS_DIR / "support" / "unified_frontend_contract.py",
)
CSS_LOAD_ORDER = (
    "app.css",
    "log_layout.css",
...
def _python_responsibility_modules() -> list[Path]:
    modules = [TESTS_DIR / "e2e" / "web" / "test_browser_journeys.py"]
    ...
    modules.extend(sorted((TESTS_DIR / "contract" / "frontend").glob("test_*.py")))
