...
from tests.support.runner import TestResult, format_summary, run_categories, run_category
...
SECTION_DESCRIPTIONS = {
    "开始使用": "统一入口，可运行全部内置套件与插件测试",
    "快速反馈": "隔离且确定，适合开发过程中的高频回归",
    "系统协作": "验证组件协作以及跨入口的稳定契约",
    "用户旅程": "覆盖完整入口和真实浏览器关键路径",
    "质量保障": "守住架构、性能与发布工程质量门槛",
    "测试体系": "验证目录发现、启动器与执行基础设施",
}
...
if _PYQT6_AVAILABLE:
    ...
    class _SectionHeader(QFrame):
        def __init__(self, section_name: str, count: int):
            ...
            meta = QLabel(SECTION_DESCRIPTIONS.get(section_name, "可独立选择或组合运行"))
    ...
    class LauncherWindow(QMainWindow):
        ...
        def _build(self):
            ...
            self.hero_sub = QLabel(
                f"{total_info['builtin_suites']} 个内置套件 / {total_info['total_files']} 个脚本；"
                f"目录决定职责，标记声明运行约束，推荐组合覆盖 {recommended_count} 个套件。"
            )
            ...
            left_title = QLabel("测试套件")
            left_title.setObjectName("panelTitle")
            left_sub = QLabel("目录定套件，标记定约束。")
            ...
            self.detail_desc = QLabel("当前尚未选择测试套件，可使用全部、推荐或自定义组合。")
            ...
            self.stat_selected = self._make_scope_metric(scope_metrics_layout, "0", "已选套件")
        ...
        def _build_selection_snapshot(self) -> dict:
            ...
            if not selected_categories:
                return {
                    ...
                    "mode": "未选",
                    "detail_desc": "当前尚未选择测试套件，可使用全部、推荐或自定义组合。",
            ...
            return {
                ...
                "unique_files": unique_files,
                "mode": "组合",
                "detail_desc": "当前执行范围为多套件组合，运行时按所选顺序依次执行。",
                "detail_tags": f"{names}  ·  共 {len(unique_files)} 个去重脚本",
                "status": f"执行范围：组合 / {len(selected_categories)} 个套件 / {len(unique_files)} 个脚本",
            }
