...
from tests.support.unified_frontend_contract import (
...
class UnifiedFrontendI18nLogsContractTests(_UnifiedFrontendContractTestCase):
    ...
    def test_gui_log_center_localizes_dynamic_log_message_and_event_code(self):
        ...
        self.assertEqual(
            localize_log_text("⚠️ 页面访问第 2/3 次加载返回网络错误页", "en-US"),
            "⚠️ Page navigation load attempt 2/3 returned a network error page",
        )
        self.assertEqual(
            localize_log_text("❌ 快手页面连续加载失败，已停止本次登录等待；请稍后重试", "en-US"),
        ...
        self.assertEqual(
            ...
            "ℹ️ HTTP did not return a direct video URL; switching to a headless browser to continue parsing",
        )
        self.assertEqual(
            localize_log_text("🛡️ 检测到 Cloudflare，请在浏览器中完成人工验证...", "en-US"),
            "🛡️ Cloudflare detected; complete the manual verification in the browser...",
        )
        self.assertEqual(
            localize_log_text("❌ Cloudflare verification timed out", "zh-CN"),
            "❌ Cloudflare 验证等待超时",
        )
    ...
    def test_gui_translation_catalog_is_loaded_from_language_files(self):
        ...
        catalog_dir = PROJECT_ROOT / "app" / "ui" / "i18n"
