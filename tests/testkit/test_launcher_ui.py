...
class TestLauncherWindowUITests(unittest.TestCase):
    ...
    def test_build_gui_exposes_core_controls(self):
        ...
        try:
            ...
            self.assertIn("目录定套件，标记定约束。", label_texts)
            self.assertIn("已选套件", label_texts)
    ...
    def test_launcher_presents_all_canonical_suites_with_truthful_group_counts(self):
        from PyQt6.QtWidgets import QLabel
        from tests import launcher
        from tests.support.catalog import BUILTIN_SUITE_ROOTS

        window = launcher._build_gui()
        try:
            expected_ids = {"all", *BUILTIN_SUITE_ROOTS}
            self.assertEqual(set(window.cards), expected_ids)
            self.assertIn("8 个内置套件", window.hero_sub.text())

            label_texts = {label.text() for label in window.findChildren(QLabel)}
            self.assertIn("测试套件", label_texts)
            self.assertIn("目录定套件，标记定约束。", label_texts)
            self.assertNotIn("测试分类", label_texts)

            headers = window.findChildren(launcher._SectionHeader)
            self.assertGreater(len(headers), 0)
            self.assertEqual(sum(header.category_count for header in headers), len(expected_ids))
            self.assertEqual(
                {header.section_name: header.category_count for header in headers},
                {
                    "开始使用": 1,
                    "快速反馈": 1,
                    "系统协作": 2,
            ...
            for suite_id in BUILTIN_SUITE_ROOTS:
                meta = window.cards[suite_id].findChild(QLabel, "categoryMetaLine")
                self.assertIsNotNone(meta)
                self.assertIn("目录套件", meta.text())
                self.assertNotIn("· suite", meta.text())
        finally:
            window.close()

    def test_category_scroll_has_clear_track_and_no_horizontal_overflow(self):
        from PyQt6.QtCore import QMargins, Qt
        from tests import launcher

        window = launcher._build_gui()
        try:
            window.resize(*launcher._launcher_minimum_size())
            window.show()
            for _ in range(5):
                self.app.processEvents()
                window._refresh_text_minimums()

            scroll = window.category_scroll
            self.assertEqual(
                scroll.horizontalScrollBarPolicy(),
                Qt.ScrollBarPolicy.ScrollBarAlwaysOff,
            )
            self.assertEqual(scroll.horizontalScrollBar().maximum(), 0)
            self.assertEqual(scroll.viewportMargins(), QMargins(0, 0, 6, 0))
            self.assertGreater(scroll.viewport().width(), 0)

            qss = launcher._launcher_qss(window.is_dark_theme)
            self.assertIn("QScrollBar::add-page:vertical", qss)
            self.assertIn("QScrollBar::sub-page:vertical", qss)
            page_rule = qss[qss.index("QScrollBar::add-page:vertical") :]
            page_rule = page_rule[: page_rule.index("}") + 1]
            self.assertIn("background: transparent", page_rule)
        finally:
            window.close()
    ...
    def test_launcher_status_bar_uses_gui_status_light_and_result_metrics(self):
        from tests import launcher
        from tests.support.runner import TestResult
