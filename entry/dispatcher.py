...
def _dispatch(mode: Mode, argv: Sequence[str]) -> int:
    ...
    if _should_delegate_to_frozen_console(mode):
        return _spawn_frozen_console_mode(mode, argv)
...
def _should_delegate_to_frozen_console(mode: Mode) -> bool:
    """带窗口的冻结入口不能在当前进程执行依赖标准输入输出的模式。"""

    if mode not in _CONSOLE_REQUIRED_MODES or not bool(getattr(sys, "frozen", False)):
        return False
    return Path(sys.executable).name.casefold() != FROZEN_CONSOLE_EXE_NAME.casefold()


def _spawn_frozen_console_mode(mode: Mode, argv: Sequence[str]) -> int:
    install_root = Path(sys.executable).resolve().parent
    console_exe = install_root / FROZEN_CONSOLE_EXE_NAME
    ...
    try:
        ...
        stream = getattr(sys, "stderr", None)
        if stream is not None:
            stream.write(f"error: failed to start frozen console launcher: {exc}\n")
        return 2
    return 0
