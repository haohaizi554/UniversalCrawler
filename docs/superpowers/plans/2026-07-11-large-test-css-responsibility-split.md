...
- [x] 建立迁移前测试方法清单，记录两个原始类中的 `test_*` 方法名。
- [x] 新增浏览器 case 模块不得被独立收集的守卫。
- [x] 新增测试 support 模块不得定义 `test_*` 方法的守卫。
- [x] 新增目标文件硬上限守卫：测试模块 1500 行、CSS 模块 1000 行。
- [x] 新增 CSS 资产必须由 `index.html` 按固定顺序且仅一次引用的守卫。
- [x] 运行守卫测试，确认在尚未拆分时只有预期的规模断言失败。
...
- [x] 将 Playwright 检测、本地服务、HTTP 就绪等待等模块级辅助函数迁入 support。
- [x] 将 `setUpClass`、`tearDownClass`、页面重置、服务重启、导航就绪和平台选项等待迁入共享基类。
- [x] 保持静态资源、WebSocket、可访问性和设计规范测试仍在入口文件。
- [x] 保持 `WebUIBrowserTests` 类名和测试节点前缀不变。
- [x] 运行静态测试及少量浏览器 smoke，确认服务只启动一次。
...
- [x] 逐个完整方法迁移，保持方法文本和名称不变。
- [x] case 类不继承 `unittest.TestCase`，类名不以 `Test` 开头。
- [x] 入口中的唯一 `WebUIBrowserTests` 按领域继承 mixin 和共享基类。
- [x] 处理领域私有 helper：仅单领域使用的随测试迁移，共享 helper 留在 support。
- [x] 比较迁移前后浏览器测试方法集合。
- [x] 运行全部浏览器测试；若 Playwright 不可用，只允许项目既有 skip 语义。
...
- [x] 迁移 `QApplication` 单例、shell 生命周期、日志/表格等待和 combo popup helper。
- [x] support 不持有领域断言或测试方法。
- [x] 保持每个测试独立清理 widget/shell。
- [x] 运行代表性 GUI 测试，确认 QApplication 与资源清理语义不变。
...
- [x] 壳层、布局、导航、通用表格和活动队列测试迁入 shell 模块。
- [x] 设置页、平台控件、弹窗与顶部栏设置契约迁入 settings 模块。
- [x] 语言、日志、动态翻译与日志详情迁入 i18n/logs 模块。
- [x] 四态列表、已完成、失败详情与分页迁入 task pages 模块。
- [x] Web 静态责任边界与跨端一致性断言迁入 static 模块。
- [x] 原入口只保留架构边界守卫和迁移说明，不重新导出测试类。
- [x] 比较迁移前后统一契约测试方法集合。
...
- Create: `app/web/static/log_layout.css`
- Create: `app/web/static/task_runtime.css`
...
- [x] 在原 CSS 中识别连续、语法闭合的职责区段。
- [x] 迁移前记录 CSS 规则顺序指纹。
- [x] 以连续区段迁移，不在本任务中格式化或去重。
- [x] `index.html` 使用七个显式 `<link>`，统一 cache-busting 版本。
- [x] 验证按 link 顺序拼接后的内容与迁移前 CSS 规范化内容一致。
- [x] 验证每个 CSS 文件大括号平衡且无空文件。
...
- [x] 新增共享 CSS 清单读取 helper，顺序以 `index.html` 为准。
- [x] 跨文件 CSS 断言使用拼接 bundle。
- [x] 单文件责任断言直接读取对应文件。
- [x] 静态资产测试覆盖七个 CSS 文件及缓存版本一致性。
- [x] 安装器和便携包测试验证所有 CSS 均随包发布。
...
- [x] 锁定 case/support 的命名和依赖方向。
- [x] 锁定 CSS 文件顺序、唯一引用和规模上限。
- [x] 记录各测试/CSS 模块所有权与新增代码落点。
- [x] 记录迁移后的测试数量、焦点耗时和全量耗时。
- [x] 记录禁止用固定 sleep、必须按真实资源顺序构建静态 bundle 的经验。

## Task 9：最终验证

- [x] 运行 `ruff check` 覆盖所有新增/修改 Python 文件。
- [x] 运行两个测试家族并核对方法集合。
- [x] 运行 packaging 测试。
- [x] 运行完整测试套件。
- [x] 运行 `git diff --check`。
