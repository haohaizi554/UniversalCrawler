...
- AppShell 四态列表索引不再由 `AppShell.render()` 在 GUI 线程扫描四个列表生成；`FrontendSnapshotWorker` 会在 snapshot/delta 合并完成后产出 `FrontendSnapshotResult.page_item_rows` 与 `completed_item_ids`，`MainWindow._on_frontend_snapshot_finished()` 只负责传递，`AppShell._apply_worker_page_item_indexes()` 只消费 worker 结果。兼容 fallback 仅保留给直接调用 `render()` 的旧测试和降级路径，不得作为运行态主路径。对应 guardrail 为 `tests/test_guardrails.py::UIAsyncGuardrailTests::test_frontend_snapshot_worker_materializes_page_indexes_for_app_shell`，行为测试为 `tests/test_frontend_snapshot_worker.py::FrontendSnapshotWorkerTests::test_build_frontend_snapshot_materializes_page_item_indexes_off_ui_thread`。
- 失败页展示字段由 `app/ui/viewmodels/failed_page_projection.py` 生成，并通过 `FailedPage._submit_page_request()` 提交给 `ListPageWorker`；页面层只接收 `ListPageResult` 后渲染当前页和详情，不在 UI 线程重跑日志本地化正则。
- `EventBus.wait_for_async_idle()` 已覆盖两条关键路径：在 async worker 线程内调用必须立即返回 `False`，handler 超过 deadline 时也返回 `False`。对应测试位于 `tests/test_event_bus_extended.py`。
- `FrontendLogCache` 的 tail key 已包含日志路径、文件身份、大小、mtime 和显示上限；tail 缓存使用 `persist=True` 写入 diskcache，并在文件变化时清理旧 `frontend.file_log_cache.tail.*` key。对应测试位于 `tests/test_frontend_log_cache.py`。
- `FrontendLogCache` 的缓存读写失败必须降级为 debug 日志并继续使用本次 source/tail 解析结果更新内存快照；缓存层异常不允许让 GUI/WebUI snapshot 热路径失去可用 batch。对应测试为 `tests/test_frontend_log_cache.py::test_log_cache_read_failure_downgrades_to_source_read` 和 `tests/test_frontend_log_cache.py::test_log_cache_write_failure_keeps_worker_batch_in_memory`。
...
- 2026-07-09 全量 `python -m pytest -q`：`2232 passed, 3 skipped, 7 warnings in 201.42s (0:03:21)`。
- 2026-07-09 本轮架构收束聚焦验证：`python -m pytest tests/test_frontend_log_cache.py tests/test_failed_record_store.py tests/test_log_query_worker.py tests/test_log_detail_worker.py tests/test_list_page_worker.py tests/test_frontend_snapshot_worker.py tests/test_guardrails.py::UIAsyncGuardrailTests::test_gui_hot_widgets_do_not_touch_files_cache_or_sqlite tests/test_guardrails.py::UIAsyncGuardrailTests::test_four_state_gui_pages_use_list_page_worker_batches tests/test_guardrails.py::UIAsyncGuardrailTests::test_log_center_page_does_not_classify_logs_on_ui_thread tests/test_guardrails.py::UIAsyncGuardrailTests::test_failed_page_uses_worker_display_projection_for_dynamic_logs tests/test_guardrails.py::UIAsyncGuardrailTests::test_failed_record_store_exposes_structured_sql_query tests/test_guardrails.py::UIAsyncGuardrailTests::test_frontend_state_service_file_log_cache_uses_background_worker -q`：`44 passed in 0.69s`。该验证只覆盖日志中心、失败记录、四态列表分页/投影、frontend snapshot worker 和 UI 热路径防回退，不等同于全项目全量验收。
- 2026-07-09 本轮静态检查：`ruff check app/services/frontend_log_cache.py app/services/failed_record_store.py app/ui/viewmodels/log_query_worker.py app/ui/viewmodels/log_detail_worker.py app/ui/pages/failed_page.py app/ui/pages/log_center_page.py tests/test_frontend_log_cache.py tests/test_failed_record_store.py tests/test_log_query_worker.py tests/test_log_detail_worker.py tests/test_list_page_worker.py tests/test_frontend_snapshot_worker.py tests/test_guardrails.py`：`All checks passed!`。
- 近期连续基线为 `2141 -> 2145 -> 2149 -> 2151 -> 2154 -> 2188 -> 2220 -> 2225 -> 2227 -> 2228 -> 2230 -> 2232`；耗时从 `208s -> 190s -> 117s -> 187s -> 237s -> 181s -> 135s -> 194s -> 192s -> 208s -> 203s -> 201s` 波动，当前实测约 3 分 21 秒。后续比较优化收益时优先记录精确秒数，不再只写“3 到 4 分钟”。
