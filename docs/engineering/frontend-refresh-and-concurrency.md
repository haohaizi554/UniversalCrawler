...
| Web 日志中心 | `log_query_worker.js`、`log_detail_worker.js` | 浏览器主线程提交 worker 请求并 patch 当前页/详情 | 主线程全量筛选、排序、详情 JSON 构建 | `test_web_browser.py` 日志 worker 相关断言、`test_unified_frontend_contract.py` 静态 bundle 断言 |
...
- 2026-07-10 focused：`python -m pytest tests/test_web_static_module_boundaries.py tests/test_fastapi_endpoints.py tests/test_web_browser.py tests/test_unified_frontend_contract.py tests/test_packaging.py -q`：`462 passed in 134.97s (0:02:14)`，`0 skipped`，`0 warnings`。
- 2026-07-10 full：`python -X faulthandler -m pytest -q`：`2361 passed, 3 skipped, 7 warnings in 181.28s (0:03:01)`。
