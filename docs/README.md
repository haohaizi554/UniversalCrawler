...
当前文档基线对应源码版本 `3.6.18`，包名与命令行为 `ucrawl`。日常运行优先参考 `main.py`、`entry.gui_entry`、`entry.web_entry`、`entry.cli_entry` 和 `entry.test_entry`；历史复盘中的旧入口或旧流程仅作为问题背景保留。
...
| 发布维护 | [guides/release-builder.md](guides/release-builder.md) | Qt 发布面板、无界面流水线、签名、信任锚与远端验收 |
