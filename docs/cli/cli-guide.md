...
| 参数 | 必填 | 默认 | 说明 |
|---|---|---|---|
| `--source` / `-s` | ✅ | - | plugin registry 中的平台 ID；内置含 `douyin` / `xiaohongshu` / `bilibili` / `kuaishou` / `missav` |
...
URL 是必填位置参数，`--title` 可选。download 的 `--timeout` 是整次直接
下载命令的期限，默认 300 秒。本地目录扫描没有平台语义，因此只支持
`ucrawl scan <directory>`；平台快捷入口只包含 `search` 和 `download`。

超时语义固定如下：

- search 的 `--http-timeout`：spider HTTP 请求超时。
- search / interactive 的 `--timeout`：整次命令期限，默认不限制。
- download 的 `--timeout`：整次直接下载命令期限，默认 300 秒。

### 插件目录与交互 manifest

`ucrawl platforms` 和 `ucrawl interactive` 读取 plugin registry 提供的同一
份 manifest。平台列表不在 argparse 中硬编码；外部插件注册后即可参与
`--source` choices 和平台快捷命令。

插件可通过 manifest 的 `interactive` 字段声明输入提示、示例、选项字段、
确认摘要和鉴权规则。未声明交互元数据的插件仍可运行，引导模式会使用安全的
通用提示，不会把它误判成某个内置平台。
...
- 插件 manifest（平台标识、别名、交互字段与鉴权规则）
- 命令运行时（`shared/search_command_runtime.py`、
  `shared/download_command_runtime.py`、`shared/scan_command_runtime.py`）
